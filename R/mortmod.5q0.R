mortmod.5q0 <- function(child.mort, prev, child.art=NULL, adult.art=NULL, region=1, sex=1, opt=TRUE, recal=NULL){
...
if(region==1 & sex==0){ # Africa, male
	...
	if(!is.null(adult.art)){
	  b2.m <- predict.lm(co2.5q0mod2.a.m, newdata=data.frame(cmort.a.m=child.mort, prev.a=prev, aart.a=adult.art))
	  b3.m <- predict.lm(co3.5q0mod2.a.m, newdata=data.frame(cmort.a.m=child.mort, prev.a=prev, aart.a=adult.art)) 
	}
	
	if(!is.null(adult.art)&!is.null(child.art)){
	  b2.m <- predict.lm(co2.5q0mod3.a.m, newdata=data.frame(cmort.a.m=child.mort, prev.a=prev, aart.a=adult.art, cart.a=child.art))
	  b3.m <- predict.lm(co3.5q0mod3.a.m, newdata=data.frame(cmort.a.m=child.mort, prev.a=prev, aart.a=adult.art, cart.a=child.art)) 
	}
...
if(opt==TRUE){
  out.mort.start <- intercept + b1.m*Mx.svd.scores.m[,1] + b2.m*Mx.svd.scores.m[,2] + b3.m*Mx.svd.scores.m[,3]
...
if(region==1 & sex==1){ # Africa, female
	...
	if(!is.null(adult.art)){
	  b2.f <- predict.lm(co2.5q0mod2.a.f, newdata=data.frame(cmort.a.f=child.mort, prev.a=prev, aart.a=adult.art))
	  b3.f <- predict.lm(co3.5q0mod2.a.f, newdata=data.frame(cmort.a.f=child.mort, prev.a=prev, aart.a=adult.art)) 
	}
	
	if(!is.null(adult.art)&!is.null(child.art)){
	  b2.f <- predict.lm(co2.5q0mod3.a.f, newdata=data.frame(cmort.a.f=child.mort, prev.a=prev, aart.a=adult.art, cart.a=child.art))
	  b3.f <- predict.lm(co3.5q0mod3.a.f, newdata=data.frame(cmort.a.f=child.mort, prev.a=prev, aart.a=adult.art, cart.a=child.art)) 
	}
...
## optimize reduction or addition to 1q0 and 4q1 to make it match
if(opt==TRUE){
  out.mort.start <- intercept + b1.f*Mx.svd.scores.f[,1] + b2.f*Mx.svd.scores.f[,2] + b3.f*Mx.svd.scores.f[,3]
...
if(region==0 & sex==0){ # Non-Africa, male
	...
	if(!is.null(adult.art)){
	  b2.m <- predict.lm(co2.5q0mod2.na.m, newdata=data.frame(cmort.na.m=child.mort, prev.na=prev, aart.na=adult.art))
	  b3.m <- predict.lm(co3.5q0mod2.na.m, newdata=data.frame(cmort.na.m=child.mort, prev.na=prev, aart.na=adult.art)) 
	}
	
	if(!is.null(adult.art)&!is.null(child.art)){
	  b2.m <- predict.lm(co2.5q0mod3.na.m, newdata=data.frame(cmort.na.m=child.mort, prev.na=prev, aart.na=adult.art, cart.na=child.art))
	  b3.m <- predict.lm(co3.5q0mod3.na.m, newdata=data.frame(cmort.na.m=child.mort, prev.na=prev, aart.na=adult.art, cart.na=child.art)) 
	}
...
if(region==0 & sex==1){ # Non-Africa, female
	...
	if(!is.null(adult.art)){
	  b2.f <- predict.lm(co2.5q0mod2.na.f, newdata=data.frame(cmort.na.f=child.mort, prev.na=prev, aart.na=adult.art))
	  b3.f <- predict.lm(co3.5q0mod2.na.f, newdata=data.frame(cmort.na.f=child.mort, prev.na=prev, aart.na=adult.art)) 
	}
	
	if(!is.null(adult.art)&!is.null(child.art)){
	  b2.f <- predict.lm(co2.5q0mod3.na.f, newdata=data.frame(cmort.na.f=child.mort, prev.na=prev, aart.na=adult.art, cart.na=child.art))
	  b3.f <- predict.lm(co3.5q0mod3.na.f, newdata=data.frame(cmort.na.f=child.mort, prev.na=prev, aart.na=adult.art, cart.na=child.art)) 
	}
