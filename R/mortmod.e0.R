mortmod.e0 <- function(e0, prev, child.art=NULL, adult.art=NULL, region=1, sex=1, opt=TRUE, recal=NULL, determ=TRUE){	
  if(!is.null(recal)){for(f in recal) load(file=f)} 
  
  lt.mx <- function (nmx, sex="female", age = ages, nax=NULL){
    n <- c(diff(age), 999)
    if(is.null(nax)){
      nax <- 0.5*n
      if(n[2]==4){
        if(sex == "male"){
          if (nmx[1] >= 0.107) {
            nax[1] <- 0.33
            nax[2] <- 1.352
          } else {
            nax[1] <- 0.045 + 2.684 * nmx[1]
            nax[2] <- 1.651 - 2.816 * nmx[1]
          } # else
        } # if (sex == "male")
        if(sex == "female"){
          if (nmx[1] >= 0.107) {
            nax[1] <- 0.35
            nax[2] <- 1.361
          } else {
            nax[1] <- 0.053 + 2.8 * nmx[1]
            nax[2] <- 1.522 - 1.518 * nmx[1]
          } # else
        } #if (sex == "female")
      } #if(n[2]==4)
    } #if(is.null(nax))
    
    nqx <- (n * nmx)/(1 + (n - nax) * nmx)
    nqx <- c(nqx[-(length(nqx))], 1)
    for (i in 1:length(nqx)) {
      if (nqx[i] > 1) 
        nqx[i] <- 1
    }
    nage <- length(age)
    nqx <- round(nqx, 4)
    npx <- 1 - nqx
    l0 = 1e+05
    lx <- round(cumprod(c(l0, npx)))
    ndx <- -diff(lx)
    lxpn <- lx[-1]
    nLx <- n * lxpn + ndx * nax
    Tx <- c(rev(cumsum(rev(nLx[-length(nLx)]))),0)
    lx <- lx[1:length(age)]
    ex <- Tx/lx
    lt <- cbind(Age = age, nax = c(round(nax[-length(nax)], 3),NA), nmx = round(nmx,4), nqx = round(nqx, 4), npx = round(npx, 4), ndx = ndx, 
                lx = lx, nLx = c(round(nLx[-length(nLx)]),NA), Tx = c(round(Tx[-length(Tx)]),NA), ex = c(round(ex[-length(ex)],2),NA))
    lt <- lt[lt[, 6] != 0, ]
    e0 <- lt[1, 10]
    lt.45q15 <- 1 - (lx[which(age==60)]/lx[which(age==15)])
    lt.5q0 <- 1 - (lx[which(age==5)]/lx[which(age==0)])
    
    return(list(e0 = e0, lt.5q0 = lt.5q0, lt.45q15 = lt.45q15, 
                lt = lt))
  }
  
  if(determ==FALSE){
    for(p in 1:length(mu.age.m)){
      if(p==1){
        ## mu is 0 at each age
        to.add.m <- rnorm(n=1, mean=mu.age.m[p], sd=sd.age.m[p])
        to.add.f <- rnorm(n=1, mean=mu.age.f[p], sd=sd.age.f[p])
      } else {
        to.add.m <- c(to.add.m, rnorm(n=1, mean=mu.age.m[p], sd=sd.age.m[p]))
        to.add.f <- c(to.add.f, rnorm(n=1, mean=mu.age.f[p], sd=sd.age.f[p]))
      }
    } # for loop
  } # if
  
if(region==1 & sex==0){ # Africa, male
	intercept <- median(svd.coeffs.xp.m[africa.nums,1])
	b1.m <- predict.lm(co1.e0mod.a.m, newdata=data.frame(le.a.m=e0))
	b2.m <- predict.lm(co2.e0mod.a.m, newdata=data.frame(le.a.m=e0, prev.a=prev))
	b3.m <- predict.lm(co3.e0mod.a.m, newdata=data.frame(le.a.m=e0, prev.a=prev))
  
	if(!is.null(adult.art)){
	  b2.m <- predict.lm(co2.e0mod2.a.m, newdata=data.frame(le.a.m=e0, prev.a=prev, aart.a=adult.art))
	  b3.m <- predict.lm(co3.e0mod2.a.m, newdata=data.frame(le.a.m=e0, prev.a=prev, aart.a=adult.art)) 
	}
	
	if(!is.null(adult.art)&!is.null(child.art)){
	  b2.m <- predict.lm(co2.e0mod3.a.m, newdata=data.frame(le.a.m=e0, prev.a=prev, aart.a=adult.art, cart.a=child.art))
	  b3.m <- predict.lm(co3.e0mod3.a.m, newdata=data.frame(le.a.m=e0, prev.a=prev, aart.a=adult.art, cart.a=child.art)) 
	}
	
	if(opt==FALSE){
	out.mort <- intercept + b1.m*Mx.svd.scores.m[,1] + b2.m*Mx.svd.scores.m[,2] + b3.m*Mx.svd.scores.m[,3]
	if(determ==FALSE){
	  out.mort <- out.mort+to.add.m
	}
  } # if
	
## optimize the intercent when predicting the mortality rates from the weights
	if(opt==TRUE){
		out.mort.func <- function(intercept.alter){
		out.mort <- intercept.alter + b1.m*Mx.svd.scores.m[,1] + b2.m*Mx.svd.scores.m[,2] + b3.m*Mx.svd.scores.m[,3]
		if(determ==FALSE){
		  out.mort <- out.mort+to.add.m
		}
#   lt.out <- LifeTableMx(mx=exp(out.mort), sex="Male")
    lt.out <- lt.mx(nmx=exp(out.mort), sex="male", age=ages, nax=NULL)
#		lt.e0 <- (rev(cumsum(rev(lt.out$Lx)))/lt.out$lx)[1]
    lt.e0 <- lt.out$e0
		e0.diff <- abs(e0-lt.e0)
	return(e0.diff)	
	} # function to be optimized
	
	intercept.opt <- optimize(f=out.mort.func, interval=c(-2,2))$minimum
	
	out.mort <- intercept.opt + b1.m*Mx.svd.scores.m[,1] + b2.m*Mx.svd.scores.m[,2] + b3.m*Mx.svd.scores.m[,3]} # if
    if(determ==FALSE){
      out.mort <- out.mort+to.add.m
    }
return(exp(out.mort))
}

if(region==1 & sex==1){ # Africa, female
	intercept <- median(svd.coeffs.xp.f[africa.nums,1])
	b1.f <- predict.lm(co1.e0mod.a.f, newdata=data.frame(le.a.f=e0))
	b2.f <- predict.lm(co2.e0mod.a.f, newdata=data.frame(le.a.f=e0, prev.a=prev))
	b3.f <- predict.lm(co3.e0mod.a.f, newdata=data.frame(le.a.f=e0, prev.a=prev))
  
	if(!is.null(adult.art)){
	  b2.f <- predict.lm(co2.e0mod2.a.f, newdata=data.frame(le.a.f=e0, prev.a=prev, aart.a=adult.art))
	  b3.f <- predict.lm(co3.e0mod2.a.f, newdata=data.frame(le.a.f=e0, prev.a=prev, aart.a=adult.art)) 
	}
	
	if(!is.null(adult.art)&!is.null(child.art)){
	  b2.f <- predict.lm(co2.e0mod3.a.f, newdata=data.frame(le.a.f=e0, prev.a=prev, aart.a=adult.art, cart.a=child.art))
	  b3.f <- predict.lm(co3.e0mod3.a.f, newdata=data.frame(le.a.f=e0, prev.a=prev, aart.a=adult.art, cart.a=child.art)) 
	}
	
	if(opt==FALSE){
	out.mort <- intercept + b1.f*Mx.svd.scores.f[,1] + b2.f*Mx.svd.scores.f[,2] + b3.f*Mx.svd.scores.f[,3]
	if(determ==FALSE){
	  out.mort <- out.mort+to.add.f
	}
	}
	
## optimize the intercept when predicting the mortality rates from the weights
if(opt==TRUE){
  out.mort.func <- function(intercept.alter){
    out.mort <- intercept.alter + b1.f*Mx.svd.scores.f[,1] + b2.f*Mx.svd.scores.f[,2] + b3.f*Mx.svd.scores.f[,3]
    if(determ==FALSE){
      out.mort <- out.mort+to.add.f
    }
    #   lt.out <- LifeTableMx(mx=exp(out.mort), sex="Male")
    lt.out <- lt.mx(nmx=exp(out.mort), sex="female", age=ages, nax=NULL)
    #  	lt.e0 <- (rev(cumsum(rev(lt.out$Lx)))/lt.out$lx)[1]
    lt.e0 <- lt.out$e0
    e0.diff <- abs(e0-lt.e0)
    return(e0.diff)	
  } # function to be optimized
  
  intercept.opt <- optimize(f=out.mort.func, interval=c(-2,2))$minimum
  
  out.mort <- intercept.opt + b1.f*Mx.svd.scores.f[,1] + b2.f*Mx.svd.scores.f[,2] + b3.f*Mx.svd.scores.f[,3]} # if
    if(determ==FALSE){
      out.mort <- out.mort+to.add.f
    }	
return(exp(out.mort))
}

if(region==0 & sex==0){ # Non-Africa, male
	intercept <- median(svd.coeffs.xp.m[la.nums,1])
	b1.m <- predict.lm(co1.e0mod.na.m, newdata=data.frame(le.na.m=e0))
	b2.m <- predict.lm(co2.e0mod.na.m, newdata=data.frame(le.na.m=e0, prev.na=prev))
	b3.m <- predict.lm(co3.e0mod.na.m, newdata=data.frame(le.na.m=e0, prev.na=prev))
  
	if(!is.null(adult.art)){
	  b2.m <- predict.lm(co2.e0mod2.na.m, newdata=data.frame(le.na.m=e0, prev.na=prev, aart.na=adult.art))
	  b3.m <- predict.lm(co3.e0mod2.na.m, newdata=data.frame(le.na.m=e0, prev.na=prev, aart.na=adult.art)) 
	}
	
	if(!is.null(adult.art)&!is.null(child.art)){
	  b2.m <- predict.lm(co2.e0mod3.na.m, newdata=data.frame(le.na.m=e0, prev.na=prev, aart.na=adult.art, cart.na=child.art))
	  b3.m <- predict.lm(co3.e0mod3.na.m, newdata=data.frame(le.na.m=e0, prev.na=prev, aart.na=adult.art, cart.na=child.art)) 
	}
	
	if(opt==FALSE){
	out.mort <- intercept + b1.m*Mx.svd.scores.m[,1] + b2.m*Mx.svd.scores.m[,2] + b3.m*Mx.svd.scores.m[,3]
	if(determ==FALSE){
	  out.mort <- out.mort+to.add.m
	}
	}
	
## optimize the intercent when predicting the mortality rates from the weights
if(opt==TRUE){
  out.mort.func <- function(intercept.alter){
    out.mort <- intercept.alter + b1.m*Mx.svd.scores.m[,1] + b2.m*Mx.svd.scores.m[,2] + b3.m*Mx.svd.scores.m[,3]
    if(determ==FALSE){
      out.mort <- out.mort+to.add.m
    }
    #   lt.out <- LifeTableMx(mx=exp(out.mort), sex="Male")
    lt.out <- lt.mx(nmx=exp(out.mort), sex="male", age=ages, nax=NULL)
    #  	lt.e0 <- (rev(cumsum(rev(lt.out$Lx)))/lt.out$lx)[1]
    lt.e0 <- lt.out$e0
    e0.diff <- abs(e0-lt.e0)
    return(e0.diff)	
  } # function to be optimized
  
  intercept.opt <- optimize(f=out.mort.func, interval=c(-2,2))$minimum
  
  out.mort <- intercept.opt + b1.m*Mx.svd.scores.m[,1] + b2.m*Mx.svd.scores.m[,2] + b3.m*Mx.svd.scores.m[,3]} # if
      if(determ==FALSE){
        out.mort <- out.mort+to.add.m
      }
return(exp(out.mort))
}

if(region==0 & sex==1){ # Non-Africa, female
	intercept <- median(svd.coeffs.xp.f[la.nums,1])
	b1.f <- predict.lm(co1.e0mod.na.f, newdata=data.frame(le.na.f=e0))
	b2.f <- predict.lm(co2.e0mod.na.f, newdata=data.frame(le.na.f=e0, prev.na=prev))
	b3.f <- predict.lm(co3.e0mod.na.f, newdata=data.frame(le.na.f=e0, prev.na=prev))
  
	if(!is.null(adult.art)){
	  b2.f <- predict.lm(co2.e0mod2.na.f, newdata=data.frame(le.na.f=e0, prev.na=prev, aart.na=adult.art))
	  b3.f <- predict.lm(co3.e0mod2.na.f, newdata=data.frame(le.na.f=e0, prev.na=prev, aart.na=adult.art)) 
	}
	
	if(!is.null(adult.art)&!is.null(child.art)){
	  b2.f <- predict.lm(co2.e0mod3.na.f, newdata=data.frame(le.na.f=e0, prev.na=prev, aart.na=adult.art, cart.na=child.art))
	  b3.f <- predict.lm(co3.e0mod3.na.f, newdata=data.frame(le.na.f=e0, prev.na=prev, aart.na=adult.art, cart.na=child.art)) 
	}
	
	if(opt==FALSE){
	out.mort <- intercept + b1.f*Mx.svd.scores.f[,1] + b2.f*Mx.svd.scores.f[,2] + b3.f*Mx.svd.scores.f[,3]
	if(determ==FALSE){
	  out.mort <- out.mort+to.add.f
	}
	}
	
## optimize the intercent when predicting the mortality rates from the weights
if(opt==TRUE){
  out.mort.func <- function(intercept.alter){
    out.mort <- intercept.alter + b1.f*Mx.svd.scores.f[,1] + b2.f*Mx.svd.scores.f[,2] + b3.f*Mx.svd.scores.f[,3]
    if(determ==FALSE){
      out.mort <- out.mort+to.add.f
    }
    #   lt.out <- LifeTableMx(mx=exp(out.mort), sex="Male")
    lt.out <- lt.mx(nmx=exp(out.mort), sex="female", age=ages, nax=NULL)
    #  	lt.e0 <- (rev(cumsum(rev(lt.out$Lx)))/lt.out$lx)[1]
    lt.e0 <- lt.out$e0
    e0.diff <- abs(e0-lt.e0)
    return(e0.diff)  
  } # function to be optimized
  
  intercept.opt <- optimize(f=out.mort.func, interval=c(-2,2))$minimum
  
  out.mort <- intercept.opt + b1.f*Mx.svd.scores.f[,1] + b2.f*Mx.svd.scores.f[,2] + b3.f*Mx.svd.scores.f[,3]} # if
  if(determ==FALSE){
    out.mort <- out.mort+to.add.f
  }
return(exp(out.mort))
}
}
