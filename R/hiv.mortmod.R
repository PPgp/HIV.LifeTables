hiv.mortmod <-
function(e0 = NULL, child.mort = NULL, adult.mort = NULL, prev = NULL, 
          child.art = NULL, adult.art = NULL, model = 1, region = 1, 
          sex = 1, lt = FALSE, opt = TRUE, recal = NULL){
#data(HIV-MLTs-obs)
# e0
if(model==1){
	mx.out <- mortmod.e0(e0 = e0, prev = prev, child.art = child.art, 
                         adult.art = adult.art, region = region, sex = sex, 
                         opt = opt, recal = recal)
}
# 5q0
if(model==2){
	mx.out <- mortmod.5q0(child.mort = child.mort, prev = prev, 
                          child.art = child.art, adult.art = adult.art, region = region, 
                          sex = sex, opt = opt, recal = recal)
}
# 5q0 and 45q15
if(model==3){
	mx.out <- mortmod.45q15(child.mort = child.mort, adult.mort = adult.mort, 
                            prev = prev, child.art = child.art, adult.art = adult.art, 
                            region = region, sex = sex, opt = opt, recal = recal)
}
...
if(lt==TRUE){
    ...
    lt.out <- lt.mx(nmx=mx.out, sex=sex.tab, age=ages)$lt
    return(lt.out)
} else {
return(mx.out)}
}